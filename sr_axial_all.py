...
if __name__ == "__main__":
    ...
    for k in datadict.keys():
        ...
        for slice_id in datadict[k]:
            print('Inference on case', case_id, 'slice', slice_id)
            val_data = val_dataset[slice_id - 1]
            data_iter = iter(val_loader)
            val_data = next(itertools.islice(data_iter, slice_id - 1, None))
            ...
            Image.fromarray(inf_img[0]).convert('L').save('{}/{}_case_{}_slice_{}_ref.png'.format(result_path, current_step, case_id, slice_id))
            Image.fromarray(sr_img[0]).convert('L').save('{}/{}_case_{}_slice_{}_sr.png'.format(result_path, current_step, case_id, slice_id))
