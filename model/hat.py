...
class Upsample(nn.Sequential):
    """Upsample module for width dimension only.
    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                # Convolution only affects the width (kernel size: 1x3)
                m.append(nn.Conv2d(num_feat, num_feat * 2, kernel_size=(1, 3), padding=(0, 1)))
                m.append(PixelShuffle1D(2))  # Use custom 1D pixel shuffle for width
        elif scale == 3:
            m.append(nn.Conv2d(num_feat, num_feat * 3, kernel_size=(1, 3), padding=(0, 1)))
            m.append(PixelShuffle1D(3))  # Use custom 1D pixel shuffle for width
        else:
