    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                m.append(nn.Conv2d(num_feat, 2 * num_feat,  kernel_size=(3, 3), padding=(1, 1)))
                m.append(PixelShuffle1D(2))
        elif scale == 3:
            m.append(nn.Conv2d(num_feat, 3 * num_feat, kernel_size=(3, 3), padding=(1, 1)))
...
class SwinIR(nn.Module):
    ...
    def forward(self, x, lr_coords):
        ...
        coord_features = self.check_image_size(coord_features)
        x = x + coord_features
        # coord_features = self.check_image_size(coord_features)
